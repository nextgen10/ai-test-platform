...
/**
 * Official UBS keys + wordmark (from ai-test-platform-ubs).
 * Light: ink keys + red wordmark. Dark: red keys + white wordmark.
 */
export function UbsLogoFull({
    height = 26,
    keysColor,
    wordmarkColor,
    className,
    ...
    const keys = keysColor ?? (isLight ? '#1c1c1c' : '#E60000');
    const wordmark = wordmarkColor ?? (isLight ? '#E60000' : '#FFFFFF');
    ...
    return (
        <svg
            ...
            role="img"
        >
            <path d={UBS_WORDMARK_PATH} fill={wordmark} />
            <path d={UBS_KEYS_PATH} fill={keys} />
        </svg>
    );
}

/** Vertical hairline between the UBS logo and the product name. */
export function BrandPipe() {
    return (
        <span
            aria-hidden
            className="inline-block h-[22px] w-px shrink-0 self-center bg-[var(--col-border-illustrative)]"
        />
    );
}

/**
 * Canonical product name: Agent HUB in brand red, Platform in ink.
 */
export function ProductName({ variant = 'inline', inverse = false }) {
    const platformTone = inverse ? 'text-[#F9F9F7]' : 'text-ink';

    if (variant === 'hero') {
        return (
            <span className="block text-[2.5rem] font-light leading-[0.95] tracking-[-0.03em] sm:text-[3.5rem] md:text-[4.75rem] lg:text-[5.5rem]">
                <span className="block text-brand">Agent HUB</span>
                <span className={cx('block', platformTone)}>Platform</span>
            </span>
        );
    }
    ...
    return (
        <span
            className={cx(
                'inline whitespace-nowrap leading-[1.15]',
                variant === 'nav' ? 'text-[0.8125rem] md:text-[0.9375rem]' : 'text-inherit',
            )}
        >
            <span className="font-medium text-brand">Agent HUB</span>
            <span className={cx('font-medium', platformTone)}> Platform</span>
        </span>
    );
}

/** UBS section kicker: a 2px brand rule and an overline in Medium. */
export function UbsEyebrow({ children, align = 'left' }) {
    return (
        <div className={cx('mb-3 flex items-center gap-3', align === 'center' && 'justify-center')}>
            <span className="h-0.5 w-6 shrink-0 bg-brand" />
            <span className="ui-overline block leading-none text-subtle">{children}</span>
        </div>
    );
}

/** UBS mark, optional hairline pipe and the product wordmark. */
export function UnifiedBrand({ onClick, showProductName = true }) {
    const { mode } = useThemeMode();
    const isLight = mode !== 'dark';
    ...
    return (
        <Tag
            ...
            onClick={onClick}
            className={cx(
                'flex min-w-0 select-none items-center gap-2 overflow-hidden sm:gap-3',
        ...
        >
            <span className="flex shrink-0 items-center">
                <UbsLogoFull
                    height={28}
                    keysColor={isLight ? '#1c1c1c' : '#E60000'}
                    wordmarkColor={isLight ? '#E60000' : '#FFFFFF'}
                />
            </span>
            {showProductName && (
                <>
