...
import { ACTIVE_STATUSES, formatDuration } from './api';
 ...
 */
function buildPhases(job, workflow, recorded) {
    const phases = [];
    const seen = new Set();
 ...
 * light the same row.
 */
function resolvePhaseKey(name, phases, workflow) {
...
export function derivePhases(job, workflow) {
    const recorded = job.provenance?.stages ?? [];
    ...
    if (states['ocr-extractor'] && workflow?.runner === 'bespoke') {
        const usedOcr =
            (job.events ?? []).some((e) => e.event_metadata?.phase === 'ocr-extractor') ||
    ...
    for (const event of job.events ?? []) {
        ...
        const detail = tidy(event.event_metadata?.detail ?? '');
