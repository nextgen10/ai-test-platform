 ...
 */
export function parseGeneratedFiles(markdown) {
    const files = [];
    ...
    for (const line of lines) {
        ...
        const heading = line.match(
            /^#{1,6}\s+`?([^`\s]+\.(?:agent\.md|workflow\.yaml|prompt\.md)|[^`\s]*SKILL\.md)`?\s*$/,
        );
        if (heading) {
 ...
 */
export function installOrder(files) {
    const rank = { skill: 0, agent: 1, prompt: 2, workflow: 3 };
    return [...files].sort((a, b) => rank[a.kind] - rank[b.kind]);
}

/** The workflow id in a generated bundle, if it produced one. */
export function bundleWorkflowId(files) {
